...
class GetRoomsUseCaseTest {
    ...
    @Test
    fun `given when remoteRepository getRooms is successful, then the required methods should be executed `() =
        runTest {
    ...
    @Test
    fun `given when remoteRepository getRooms is not successful and localRepository is not empty, then the required methods should be executed `() =
        runTest {
            val errorMessage = "An error occurred"
            val mockedRoom1 = MockUtils.createMockRoomEntity("1")
            val mockedRoom2 = MockUtils.createMockRoomEntity("2")

            coEvery { remoteRepository.getRooms() } returns ApiResult.NetworkError(
                Exception(
                    errorMessage
                )
            )
    ...
    @Test
    fun `given when remoteRepository getRooms is not successful and localRepository is empty, then the required methods should be executed `() =
        runTest {
            val errorMessage = "An error occurred"
