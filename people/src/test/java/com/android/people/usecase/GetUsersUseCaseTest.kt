package com.android.people.usecase
...
import com.android.people.utils.MockUtils
...
class GetUsersUseCaseTest {
    ...
    @Test
    fun `given when remoteRepository getUsers is successful, then the required methods should be executed `() =
        runTest {
    ...
    @Test
    fun `given when remoteRepository getUsers is not successful and localRepository is not empty, then the required methods should be executed `() =
        runTest {
            val errorMessage = "An error occurred"
            val mockedUser1 = MockUtils.createMockUserEntity("1")
            val mockedUser2 = MockUtils.createMockUserEntity("2")

            coEvery { remoteRepository.getUsers() } returns ApiResult.NetworkError(
                Exception(
                    errorMessage
                )
            )
    ...
    @Test
    fun `given when remoteRepository getUsers is not successful and localRepository is empty, then the required methods should be executed `() =
        runTest {
            val errorMessage = "An error occurred"
